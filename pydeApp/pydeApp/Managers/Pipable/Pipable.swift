...
//               
//               // 创建圆形路径（在边界内居中）
//            let circlePath = UIBezierPath(ovalIn: CGRect(x: 10, y: 10, width: 50, height: 50))
//               
//               // 填充路径
//               circlePath.fill()
            
        }
//        try? image.pngData()?.write(to: ConstantManager.documentURL.appendingPathComponent("test.png"))
//        
//        guard let window else { return image}
//
//        let renderer1 = UIGraphicsImageRenderer(size: window.bounds.size)
//        let screenshot = renderer1.image { ctx in
//            window.drawHierarchy(in: window.bounds, afterScreenUpdates: true)
//        }
//        try? screenshot.pngData()?.write(to: ConstantManager.documentURL.appendingPathComponent("test2.png"))
