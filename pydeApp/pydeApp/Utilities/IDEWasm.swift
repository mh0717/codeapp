...
//    config.setURLSchemeHandler(PYFileURLSchemeHandler(), forURLScheme: "file")
//    let httpProxy = PYHTTPSchemeHandler(baseFileUrl: ConstantManager.WASM)
//    config.setURLSchemeHandler(httpProxy, forURLScheme: "http")
    return config
}
...
@objc class PYFileURLSchemeHandler: NSObject, WKURLSchemeHandler {
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        ...
        if url.scheme == "file" {
            ...
            let fileUrlRequest = URLRequest(url: url)
            let session = URLSession.shared
            let task = session.dataTask(with: fileUrlRequest) { data, response, error in
                if let error {
                ...
                if let data, let response = HTTPURLResponse(url: url, statusCode: 200, httpVersion: "HTTP/1.1", headerFields: headerFields) {
                    urlSchemeTask.didReceive(response)
...
@objc class PYHTTPSchemeHandler: NSObject, WKURLSchemeHandler {
    let baseFileUrl: URL
    ...
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        ...
        let fileUrl = baseFileUrl.appendingPathComponent(url.path)
        if url.scheme == "http" {
            var isDirectory: ObjCBool = false
